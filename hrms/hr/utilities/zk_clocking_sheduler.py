...
def get_list_of_employee_pins():
    employees = []
    employee_list = frappe.get_all('Employee', 'attendance_device_id', { 
        'status': 'Active',
...
def get_employees_clockings_from_zk_since_last_sync(pids):
    ...
    sql = "select id, emp_code `pin`, punch_time `checktime`, punch_state `checktype`, terminal_sn `sn_name` From iclock_transaction\
           ...
           and punch_time >= %s \
           order by punch_time" \
           % (hr_settings.last_sync_id,','.join(pids), formatdate(hr_settings.import_after_date, 'yyyy-mm-dd'))
    results = None
    try:
        cursor.execute(sql)
        # Fetch all the rows in a list of lists.
